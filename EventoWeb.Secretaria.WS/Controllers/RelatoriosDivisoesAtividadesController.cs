...
namespace EventoWeb.Secretaria.WS.Controllers
{
    ...
    public class RelatoriosDivisoesAtividadesController : ControllerBase
    {
        ...
        [Authorize("Bearer")]
        public async Task<ActionResult> GerarPdf(long actividadeId, [FromQuery] bool detalhar = false)
        {
            try
            {
                var pdf = await _appRelatorio.GerarRelatorioAsync(actividadeId, detalhar);
                
                return File(pdf, "application/pdf", $"relatorio-divisoes-atividade-{actividadeId}.pdf");
            }
